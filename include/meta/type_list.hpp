...
namespace world::meta {
    ...
    // Add / remove components. TODO: check for duplicates.
    template<typename List, typename T> using add_t = typename sort_types<typename List::template prepend<T>>::type;
    template<typename T, typename List>
    struct remove;

    template<typename T>
    struct remove<T, type_list<>> {
        using type = type_list<>;
    };

    template<typename T, typename Head, typename... Tail>
    struct remove<T, type_list<Head, Tail...>> {
        using rest = typename remove<T, type_list<Tail...>>::type;
    ...
    using remove_t = typename sort_types<typename remove<T, List>::type>::type;
